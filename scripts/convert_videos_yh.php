 ...
 * 视频转换 切片
 * 
 * @param unknown $video_name
 *            视频名称 4613.mp4
 * @param int $vid
 *            视频id 4613
 * @param unknown $video_path
 *            视频磁盘路径 D:\www\labase/media/videos/vid/4613.mp4
 * @param $delogo 是否删除logo            
 * @param $thumb 缩略图            
 */
function convert_videos($video_name, $vid, $video_path, $delogo = false, $thumb = false)
{
...
//     require '/include/function_server.php';    
    ...
    if (! preg_match("/^[0-9]{1,5}\.[a-z0-9]{2,4}$/i", $video_name)) {
        echo "Video Name: $video_name is invalid. Err #1. Exiting ...";
        exit();
    } else {
